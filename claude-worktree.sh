...
#
# Convention (the only coupling between this script and the spec-workflow
# skills): the worktree directory name IS the ticket key (e.g. PROJ-123).
# `cwt PROJ-123` creates ./PROJ-123 as branch + worktree name, and this
# launcher passes that basename to /spec-workflow:orchestrator as the ticket.
...
#
# Starts Claude Code with the spec-workflow orchestrator pre-invoked on the
